...
add_action( 'plugins_loaded', 'woocommerce_thepay_init', 10 );

function woocommerce_thepay_init(){
    if ( class_exists( 'WC_pay_uniadapter' ) ) {

	    class ThePayBinder extends WC_pay_uniadapter {
		    ...
		    function __construct() {
			    ...
			    parent::__construct( 'ThePay', '' );
		    }

	    }

        function woocommerce_add_thepay_gateway($methods){
	    $methods[] = 'ThePayBinder';
            return $methods;
        }

        add_filter( 'woocommerce_payment_gateways', 'woocommerce_add_thepay_gateway' );
    }
}
