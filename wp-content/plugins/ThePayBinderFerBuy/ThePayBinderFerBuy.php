...
/*
  Plugin Name: WooCommerce ThePay FerBuy
...
add_action( 'plugins_loaded', 'woocommerce_thepayferbuy_init', 10 );

function woocommerce_thepayferbuy_init(){
    if ( class_exists( 'WC_pay_uniadapter' ) ) {

	    class ThePayBinderFerBuy extends WC_pay_uniadapter {
		    ...
		    function __construct() {
			    ...
			    parent::__construct( 'ThePay', 'FerBuy' );
		    }

	    }

        function woocommerce_add_thepayferbuy_gateway($methods){
	    $methods[] = 'ThePayBinderFerBuy';
            return $methods;
        }

        add_filter( 'woocommerce_payment_gateways', 'woocommerce_add_thepayferbuy_gateway' );
    }
}
